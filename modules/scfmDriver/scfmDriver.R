defineModule(sim, list(
  ...
  parameters = rbind(
    defineParameter("useExampleDriverPars", "logical", TRUE, NA, NA, "Should Driver restore the object from ecodistrict 644?"),
  ...
  inputObjects = bind_rows(
    ...
    expectsInput(objectName = "ecodistrict", objectClass = "numeric",
                 desc = "number of the study area's ecodistrict"),
...
doEvent.scfmDriver = function(sim, eventTime, eventType, debug = FALSE) {
  switch(
    eventType,
    init = {
      sim <- Cache(Init, sim, userTags = c("functionObjective:calibration", 
                                           paste0("EcoDistrict", sim$ecodistrict)))
    },
...
Init <- function(sim) {
  ...
  if (P(sim)$useExampleDriverPars){
    if (sim$ecodistrict != 644) stop("useExampleDrivePars can only be TRUE is used with 'ecodistrict = 644'. 
                                     Set it to FALSE if using another ecodistrict")
    message(crayon::yellow("useExampleDriverPars is TRUE. 
      This is a fixed cached object for the example study area (Ecodistrict 644)."))
    sim$scfmDriverPars <- get(load(file = file.path(dataPath(sim),"cachedDriversEcodistrict644.rda")))
  } else {
    sim$scfmDriverPars <- Cache(userTags = c("mainFunction::Map2", "objectName::scfmDrivePars"),
                                ...
                                f = function(polygonType, targetN = P(sim)$targetN,
                                  ...
                                  calibData <- Cache(executeDesign, L = calibLand$flammableMap,
                                                     dT, maxCells = maxBurnCells, userTags = "functionObjective:internalCalibration")

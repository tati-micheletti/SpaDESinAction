...
Init <- function(sim) {
  ...
  if (!identicalCRS(sim$fireRegimePolys, tmp)){
    tmp <- projectInputs(x = tmp, targetCRS = crs(sim$fireRegimePolys))
  }
    ...
    if (needNewDownload) {
      print("downloading NFDB")# put prepInputs here
        sim$firePoints <- Cache(prepInputs, url = extractURL(objectName = "firePoints"),
                                     studyArea = sim$studyArea[!(names(sim$studyArea) %in% c("ECODISTRIC", "ECOREGION"))], fun = "shapefile",

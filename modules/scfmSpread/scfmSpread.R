...
defineModule(sim, list(
  ...
  parameters = rbind(
    ...
    defineParameter(".plotInitialTime", "numeric", start(sim) + 1, NA, NA, "This describes the simulation time at which the first plot event should occur"),
  ...
  switch(
    ...
    plot = {
      ...
      if (!is.null(sim$rstCurrentBurn))
        Plot(sim$flammableMap, cols = c("skyblue", "red"), legend = TRUE, title = "Flammable Map")
      if (!is.null(sim$rstCurrentBurn)){
        Plot(sim$rstCurrentBurn, new = TRUE, col = c("grey", "red"), title = "Current burn")
      } else {
        sim$rstCurrentBurn <- sim$burnMap
